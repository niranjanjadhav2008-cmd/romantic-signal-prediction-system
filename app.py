...
CATEGORY_SVG = {
    ...
    <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
        ...
        <!-- Main chat bubble -->
        <rect x="30" y="50" width="120" height="70" rx="20" 
        ...
        <!-- Second bubble -->
        <rect x="55" y="130" width="110" height="45" rx="15" 
              fill="#c026d3" opacity="0.7"/>
    ...
    <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
        <circle cx="100" cy="100" r="90" fill="#1a1025" opacity="0.5"/>
        <!-- Phone body -->
        <rect x="65" y="30" width="70" height="140" rx="15" 
              fill="#2a1a2e" stroke="#ff4b91" stroke-width="3"/>
        <!-- Screen -->
        ...
        <text x="65" y="54" text-anchor="middle" 
              fill="white" font-size="9" font-weight="bold">3</text>
    </svg>
    """,
    ...
    <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
        ...
        <!-- Left pan -->
        <path d="M30 93 Q55 115 80 93" fill="none" 
              stroke="#ff4b91" stroke-width="3"/>
        <ellipse cx="55" cy="115" rx="25" ry="6" fill="#ff4b91" opacity="0.7"/>
        <!-- Right pan -->
        <path d="M120 93 Q145 115 170 93" fill="none" 
              stroke="#ff4b91" stroke-width="3"/>
...
if st.session_state.step>=len(questions):
    ...
    with st.spinner("Analyzing your signals..."):
        try:
            ai_analysis = generate_ai_feedback(
                percentage,
                ml_percentage,
                communication_percentage,
                emotional_bond_percentage,
                attraction_percentage,
                social_interest_percentage,
...
<div class="right-panel">
    <div class="right-panel-label">🤖 AI Insight</div>
...
with center:
    ...
    if dtype == int and rule == [0,1]:
        previous_value = st.session_state.answers.get(key,0)
        ...
        previous_value = st.session_state.answers.get(key,"rarely")
        index = options.index(previous_value.capitalize())
        choice = st.selectbox("",options,index = index,key = widget_key)
        value = choice.lower()
    else:
        previous_value = st.session_state.answers.get(key,2)
        if labels:
